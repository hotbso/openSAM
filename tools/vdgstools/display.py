...
class XPObj:
    ...
    def quad(self, txq, x, y, h2w = 1.0, anchor = "lb", scale = 1.0):
        """ anchor: lb, cb, rb, lt, ct, rt =
            left, center, right + bottom, top
        """
        x = x * self.scale
        y = y * self.scale
        w = txq.w * txq.tex.pix2m * scale
        h = txq.h * txq.tex.pix2m * h2w * scale
        ...
        if anchor == "lb":
            ...
            raise Exception(f"invalid anchor '{anchor}'")
    ...
    def hide_if_eq(self, dref, val):
        self.line(f"ANIM_hide {val:0.4f} {val:0.4f} {dref}")
        self.hides += 1
    ...
    def trans_y(self, dy_bottom, dy_top, v1, v2, dref):
        """move vertical from dy_bottom to dy_top while dref varies between v1 and v2"""
    ...
    def dump(self, new_obj_fn = None):
        if new_obj_fn is not None:
