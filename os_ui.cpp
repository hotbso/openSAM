...
// static
int MyPlane::UIWidgetCb(XPWidgetMessage msg, XPWidgetID widget_id, [[maybe_unused]] intptr_t param1,
                          intptr_t param2) {
...
void MyPlane::UpdateUI(bool only_if_visible) {
    if (ui_widget == NULL || (only_if_visible && !XPIsWidgetVisible(ui_widget))) {
        LogMsg("UpdateUI: widget is not visible");
        return;
    }

    LogMsg("UpdateUI started");
...
static void CreateUI() {
    ...
    ui_widget = XPCreateWidget(left, top, left + width, top - height, 0, "openSAM " VERSION_SHORT, 1, NULL,
                               xpWidgetClass_MainWindow);
    ui_widget_ctx.Set(ui_widget, left, top, width, height);

    XPSetWidgetProperty(ui_widget, xpProperty_MainWindowHasCloseBoxes, 1);
    XPAddWidgetCallback(ui_widget, MyPlane::UIWidgetCb);

    top -= 20;
    left1 = left + 60;
    XPCreateWidget(left1, top, left1 + 50, top - 20, 1, "Jetway selection", 0, ui_widget, xpWidgetClass_Caption);

    top -= 30;
    left1 = left + margin;

    auto_btn =
        XPCreateWidget(left1, top, left1 + 100, top - 20, 1, "Automatic Mode", 0, ui_widget, xpWidgetClass_Button);
    XPSetWidgetProperty(auto_btn, xpProperty_ButtonBehavior, xpButtonBehaviorCheckBox);
    XPAddWidgetCallback(auto_btn, MyPlane::UIWidgetCb);

    top -= 30;
    for (int i = 0; i < kMaxDoor; i++) {
        char label[50];
        sprintf(label, "Door %d", i + 1);
        XPCreateWidget(left1, top, left1 + 50, top - 20, 1, label, 0, ui_widget, xpWidgetClass_Caption);
        left1 += col_spacing;
    }
    ...
    for (int j = 0; j < kNearJwLimit; j++) {
        ...
        for (int i = 0; i < kMaxDoor; i++) {
            ...
            XPAddWidgetCallback(btn, MyPlane::UIWidgetCb);
    ...
    XPAddWidgetCallback(dock_btn, MyPlane::UIWidgetCb);
