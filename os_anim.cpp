...
float SamAnim::AnimAcc(void* ref) {
    ...
    int drf_idx = reinterpret_cast<uint64_t>(ref);
    if (drf_idx < 0 || drf_idx >= (int)SamDrf::sam_drfs.size()) {
        LogMsg("invalid drf_idx %d in SamAnim::AnimAcc", drf_idx);
        return 0.0f;
    }
...
static float AutoDrfAcc(void* ref) {
    ...
    int drf_idx = reinterpret_cast<uint64_t>(ref);
    if (drf_idx < 0 || drf_idx >= (int)SamDrf::sam_drfs.size()) {
        LogMsg("invalid drf_idx %d in AutoDrfAcc", drf_idx);
        return 0.0f;
    }
...
bool AnimInit() {
    for (unsigned i = 0; i < SamDrf::sam_drfs.size(); i++) {
        const SamDrf& drf = SamDrf::sam_drfs[i];
        void* ref = reinterpret_cast<void*>(static_cast<uint64_t>(i));
        if (drf.autoplay)
            XPLMRegisterDataAccessor(drf.name.c_str(), xplmType_Float, 0, NULL, NULL, AutoDrfAcc, NULL, NULL, NULL,
                                     NULL, NULL, NULL, NULL, NULL, NULL, ref, NULL);
        else
            XPLMRegisterDataAccessor(drf.name.c_str(), xplmType_Float, 0, NULL, NULL, SamAnim::AnimAcc, NULL, NULL, NULL, NULL,
                                     NULL, NULL, NULL, NULL, NULL, ref, NULL);
    }
