...
//
void ReadWav(const std::string& fname, Sound& sound) {
    sound = {};
    ...
    if (memcmp(header.riffheader.id, "RIFF", 4) || memcmp(header.wav, "WAVE", 4) ||
        memcmp(header.fmtheader.id, "fmt ", 4) || header.fmtheader.size < sizeof(WAV_FORMAT)) {
