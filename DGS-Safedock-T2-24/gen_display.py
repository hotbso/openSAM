...
xpo = XPObj(sys.argv[1], -0.4, -0.27, 0.49, 0.8, 0.9)
...
with AnimBlock(xpo):
    ...
    xpo.hide_if_eq(status_1_dr, 1)  # TODO until the T2 gets an idle display
...
with AnimBlock(xpo):
    ...
    with AnimBlock(xpo):
        xpo.trans_y(0, 42, 0, 1, move_dr)
        xpo.quad(lead_in_txq, LED(2), 8)
...
xpo.line("#---- status_1 = 1 -> CHOCK ON")
with AnimBlock(xpo):
    xpo.show_if_eq(status_1_dr, 1)
